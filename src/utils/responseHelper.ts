...
export const sendSuccessResponse=(res:Response,{status,message,data}:TsuccessResponse)=>{
    res.status(status).json({ok:true,message,data});
}

export const sendErrorResponse=(res:Response,{status,message,error}:TerrorResponse)=>{
